...
**Scope of this report (one original, one improved).** We focus on **a single official visualization**: the SingStat-style multi-series line chart of *proportion of resident households by type of dwelling* (shown as Figure 1). The interactive chart under @sec-original-chart is a **reproducible, interactive version of that same visualization**—not a second, unrelated original plot. Our final deliverable is **one improved visualization**: a **stacked faceted (small-multiple) line-chart figure** (see @sec-small-multiples), justified as a single graphic in the course sense because all panels share the same geom (lines over time), the same horizontal axis (year), and coherent percent-based *y*-scales.

To avoid confusion between **the figure** and **the written argument**: @sec-small-multiples presents the **improved plot** only; @sec-proposed-improvements explains **why** those design choices were made and **which weaknesses** each choice addresses.

## Understanding The Chosen Visualisation {#sec-understanding}

![Figure 1 : Proportion of Resident Households by Selected Type of Dwelling](images/figure_1.png)
...
- **Resident Households:** The total number (count) of resident households living in Singapore (used as the denominator for percentage shares).
...
This section reproduces **the same visualization** as Figure 1 (@sec-understanding) using our `data.csv`, as one interactive Plotly chart so readers can explore the full series and date range. It is **not** a separate “second original” plot. Use the start and end year controls to change the date range.
...
p <- p %>%
  layout(
    title = list(
      text = "Proportion of Resident Households by Selected Type of Dwellings",
      x = 0.5,
...
alone_wide <- read.csv("Living Alone Married without Ch.csv", check.names = FALSE)
alone_wide <- alone_wide %>% slice(2:7)
...
# Phase 2
series_std <- c(
  "Resident Households" = "Resident Households", "Total HDB Dwellings" = "Total HDB Dwellings",
  "HDB 1- And 2-Room Flats" = "HDB 1- And 2-Room Flats", "HDB 3-Room Flats" = "HDB 3-Room Flats",
  "HDB 4-Room Flats" = "HDB 4-Room Flats", "HDB 5-Room And Executive Flats" = "HDB 5-Room And Executive Flats",
  "Condominiums And Other Apartments" = "Condominiums And Other Apartments", "Landed Properties" = "Landed Properties",
  "Other Types Of Dwelling" = "Other Types Of Dwelling"
)
silo_a_long_pipe <- silo_a_wide %>%
  pivot_longer(cols = all_of(year_cols), names_to = "Year", values_to = "Value") %>%
  mutate(Year = as.numeric(Year), Series = trimws(`Data Series`), Series = recode(Series, !!!series_std)) %>%
  filter(!is.na(Value))
total_households_pipe <- silo_a_long_pipe %>% filter(Series == "Resident Households") %>% select(Year, Total = Value)
housing_shares <- silo_a_long_pipe %>%
  filter(Series != "Total HDB Dwellings") %>%
  left_join(total_households_pipe, by = "Year") %>%
...
silo_b_eng <- silo_b %>%
  left_join(total_households_pipe, by = "Year") %>%
  mutate(married_no_children_pct = if_else(!is.na(Total) & Total > 0 & !is.na(married_no_children), 100 * married_no_children / Total, NA_real_))
silo_c_eng <- silo_c %>%
  arrange(Year) %>%
  mutate(
...
silo_d_eng <- silo_d %>% mutate(affordability_ratio = median_income_index / condo_price_index)
# Phase 4
housing_wide <- housing_shares %>%
  filter(Series %in% c("HDB 3-Room Flats", "HDB 4-Room Flats", "HDB 5-Room And Executive Flats", "Condominiums And Other Apartments", "Landed Properties")) %>%
...
# Improved visualization (faceted plot) {#sec-small-multiples}

**Single improved figure.** Course guidance treats a **faceted** layout as **one visualization** when subplots share the same geoms and a coherent mapping. Here, every panel uses **line charts over calendar year** with **percent** on the vertical axis: four panels show **household share by dwelling type** (core story); the bottom panel overlays **two context lines** (1- and 2-person household share of resident households; non-resident share of total population) from the engineered data—still lines over time, aligned with the same *x*-axis narrative. This is **not** a dashboard of unrelated chart types.

Five panels: dwelling shares **HDB 3-room**, **HDB 4-room**, **HDB 5-room**, **Condo** (Silo A), then the **combined context** panel (1 & 2 person % and Non-Resident % with direct labels — Silos B & C). Condo sits directly above the context panel. Shared **Year** on *x*. Blue tones = public (HDB); warm = private / context accents. Use the date filter below to set the range.

**Rationale** for the design choices and their link to weaknesses appears in @sec-proposed-improvements.
...
```{r improved-chart}
# 4 dwelling panels (Silo A) with 1 combined context panel from master_table (Silos B & C). Landed excluded.
...
make_opt_a <- function(series_name, title, color) {
  ...
  plot_ly(d, x = ~Year, y = ~Percent, type = "scatter", mode = "lines",
    ...
    layout(
      ...
      margin = list(t = 36, b = 32, l = 44, r = 16),
      showlegend = FALSE,
      annotations = list(
        list(
          x      = last_pt$Year,
...
plist_dwelling <- lapply(1:5, function(i) make_opt_a(opt_a_series[i], opt_a_titles[i], opt_a_colors[i]))

# Combined context chart: 1 & 2 person % (Silo B) and Non-resident % (Silo C) with direct labels
mt_ctx <- master_table %>% filter(!is.na(shrinkage_index) | !is.na(non_resident_ratio))
last_shrink <- mt_ctx %>% filter(!is.na(shrinkage_index)) %>% slice_max(Year, n = 1) %>% slice(1)
last_nr     <- mt_ctx %>% filter(!is.na(non_resident_ratio)) %>% slice_max(Year, n = 1) %>% slice(1)
p_combined <- plot_ly(mt_ctx) %>%
  add_trace(x = ~Year, y = ~(shrinkage_index * 100),
    type = "scatter", mode = "lines",
    name = "1- & 2-person households (%)",
    line = list(width = 2, color = "#4a90a4"),
    hovertemplate = "<b>1 & 2 person %</b><br>Year: %{x}<br>% of households: %{y:.1f}%<extra></extra>"
  ) %>%
  add_trace(
    x = ~Year, y = ~(non_resident_ratio * 100),
    type = "scatter", mode = "lines",
    name = "Non-residents (% of population)",
    line = list(width = 2, color = "#c6531a"),
    hovertemplate = "<b>Non-resident %</b><br>Year: %{x}<br>% of population: %{y:.1f}%<extra></extra>"
  ) %>%
  layout(
    title  = list(text = "Context: 1 & 2 person households (%) & Non-residents (% pop.)", font = list(size = 12)),
    xaxis  = list(range = c(min_year, max_year), dtick = 5),
    yaxis  = list(title = "%", tickformat = ".1f"),
    margin = list(t = 36, b = 32, l = 44, r = 16),
    showlegend = FALSE,
    annotations = list(
      list(
        x = last_shrink$Year, y = last_shrink$shrinkage_index * 100,
        text = "1 & 2 person",
        showarrow = FALSE, xanchor = "right", yanchor = "bottom",
        xref = "x", yref = "y",
        font = list(size = 10, color = "#4a90a4"),
        cliponaxis = FALSE
      ),
      list(
        x = last_nr$Year, y = last_nr$non_resident_ratio * 100,
        text = "Non-Resident",
        showarrow = FALSE, xanchor = "right", yanchor = "top",
        xref = "x", yref = "y",
        font = list(size = 10, color = "#c6531a"),
        cliponaxis = FALSE
      )
    )
  )

# Order: HDB 3, HDB 4, HDB 5, Condo, [Combined] — Landed removed
plist_all <- c(
  plist_dwelling[1:4],
  list(p_combined)
)
p2 <- subplot(plist_all, nrows = 5, margin = 0.02, shareX = TRUE, shareY = FALSE) %>%
  layout(
    title = list(text = "Improved visualization (faceted): dwelling shares + context (shared year axis)", x = 0.5, font = list(size = 14)),
  ...
  htmlwidgets::onRender("
    function(el, x) {
      window.updateChartRange2 = function() {
        ...
        // Update x-axis range (shared axis)
        Plotly.relayout(el.id, {
        ...
        // Move each annotation to the last visible data point within the new range
        var layout = el._fullLayout;
        var newAnnotations = JSON.parse(JSON.stringify(el.layout.annotations || []));

        el.data.forEach(function(trace, traceIdx) {
          var xs = trace.x;
          var ys = trace.y;
          if (!xs || !ys) return;

          // Find the rightmost x value <= endYear
          var bestX = null, bestY = null;
          for (var i = 0; i < xs.length; i++) {
            if (xs[i] <= endYear && xs[i] >= startYear) {
              if (bestX === null || xs[i] > bestX) {
                bestX = xs[i];
                bestY = ys[i];
              }
            }
          ...
          // Each dwelling trace has one annotation (index matches traceIdx for first 5 traces)
          if (newAnnotations[traceIdx] !== undefined) {
            newAnnotations[traceIdx].x = bestX;
            newAnnotations[traceIdx].y = bestY;
          }
        });

        Plotly.relayout(el.id, { annotations: newAnnotations });
      };
    }
  ")
p2
...
The chart uses a black-and-blue colour scheme with varying dash patterns; while functional in a printed government report, it creates difficulties for colourblind readers and reduces distinguishability when photocopied or viewed on a grey-scale screen. The two blue lines (HDB 3-Room Flats and Landed Properties) are particularly close in hue, and the reliance on dash-pattern differentiation compounds this issue. A more accessible design might employ a broader colour palette with distinct hues and sufficient contrast ratios.
...
# Rationale: proposed improvements {#sec-proposed-improvements}

This section is **only the written rationale**. The **interactive improved figure** is in @sec-small-multiples (not here), so readers are not asked to compare two different “improved” sections.

The vis in @sec-small-multiples implements the following design moves in response to the original visualization critiqued in @sec-strengths-weaknesses.

## Mapping weaknesses to improvements {#sec-weakness-map}

| Weakness (original) | Improvement(s) in the faceted figure |
|---------------------|----------------------------------------|
| Overlapping lines / visual clutter | **Small multiples:** one dwelling series per panel; shared *x* so trends align without crossing lines. |
| Separated legend, high cognitive load | **Direct labels** at line ends on every panel; no legend. |
| Dash-pattern encoding hard to read | **Solid lines only**; categories distinguished by **hue** (blues vs orange / teal). |
| Monochrome / low accessibility | **Distinct hues** with higher contrast than black/blue + dashes. |
| Hard to read values without gridlines | **Interactive date-range filter** across all panels to zoom and read trends (partial mitigation; gridlines remain optional). |

Some improvements address **several** weaknesses at once (e.g. colour + solid lines + small multiples). The **context panel** (1 & 2 person %, Non-Resident %) mainly **adds analytical depth** (multi-dimensional story) rather than fixing a single weakness of the government chart.

---

**Small multiples and combined context panel**  
Instead of plotting all six dwelling categories in a single chart, the improved chart uses five panels: four for dwelling shares (HDB 3-room, HDB 4-room, HDB 5-room, Condo)—one series per panel—and one combined context chart showing 1 & 2 person households (%) and Non-Resident (% of population), with Condo placed directly above the combined chart. Each panel shares the same x-axis (Year) and uses percent on the y-axis where relevant, so trends can be compared across panels without overlapping lines. Landed properties are omitted to keep the focus on the main dwelling shift and context. This removes visual clutter and makes it easier to trace any single category over the full time span (Tufte's layering and separation).

**Simplified line styles and coordinated colour palette**  
All series use solid lines only (no dotted or dashed patterns), reducing the decoding effort that the original chart required. Dwelling types are differentiated by colour: shades of blue for public housing (HDB 3-, 4-, and 5-room) and warm colours (orange) for private housing (Condo). The combined context chart uses teal for 1 & 2 person % and orange for Non-Resident %. This improves distinguishability for screen and print and supports better accessibility than the original monochrome palette with pattern encoding.

**Direct labelling (no separate legend)**  
Each dwelling panel has the series name (e.g. "HDB 3-room", "Condo") annotated directly at the end of the line. The combined context panel uses direct labels "1 & 2 person" and "Non-Resident" at the end of each line, so the viewer does not need to cross-reference a legend. This reduces cognitive load and aligns with Tufte's recommendation to place labels on or adjacent to the data they describe.

**Interactive date-range filter**  
Start Year and End Year controls (with an "Update Range" button and Enter-key support) allow the user to focus on a chosen time window (e.g. 1995–2005 or 2010–2024). Narrowing the range makes it easier to read values and compare series without relying on gridlines, and the same filter applies across all five panels (four dwelling with one combined context) so comparison remains consistent.
...
The original visualisation attributes the shift in housing composition primarily to shrinking household sizes. While this is supported by the data, other contributing factors include changes in the non-resident population and growth in household incomes. The data sources below are used to contextualise these factors.

The report uses the following CSV files for the pipeline and improved chart: `data.csv` (dwelling-type counts), `Resident Household by household.csv`, `Living Alone Married without Ch.csv`, `Composition of Non-Residents.csv`, and `Gross monthly income.csv`. (The HDB Resale Flat Prices dataset is cited in the literature but not integrated in the current pipeline.) Percentages in the charts are computed relative to total resident households for each year. Full citations: Singapore Department of Statistics (SingStat), HDB sample household survey, and MOM Labour Force Report.
...
Additionally, we can refer to [SingStat Table M810651](https://tablebuilder.singstat.gov.sg/table/TS/M810651), where we can notice a rising trend of resident households living alone as well as those married without children.
...
Referring to [SingStat Table M810001](https://tablebuilder.singstat.gov.sg/table/TS/M810001), we can observe a jump in the non-resident population from 2021 to 2025.
...
![SingStat Table M810001 — Part 4](images/m810001_part4.png)
...
| **B (Demographics)** | [SingStat Table M810371](https://tablebuilder.singstat.gov.sg/table/TS/M810371) (Household Size) & [SingStat Table M810651](https://tablebuilder.singstat.gov.sg/table/TS/M810651) (Living Arrangements) | Extract **"1-person"** and **"Married without Children"** as new feature columns. |
...
```{r preview-phase1-silo-a, echo=FALSE}
yr_preview <- intersect(as.character(sort(years)[1:min(5, length(years))]), names(silo_a_wide))
knitr::kable(
  silo_a_wide %>% select(`Data Series`, all_of(yr_preview)),
  caption = "Phase 1 — excerpt: Silo A (`data.csv`), first few year columns"
)
```
...
#### Living Alone & Married without Children: 
**Living Alone Married without Ch.csv**
```{r, echo=TRUE}
...
```{r preview-phase1-silo-b, echo=FALSE}
knitr::kable(
  head(silo_b %>% select(Year, shrinkage_index, one_person_household_pct, avg_household_size), 6),
  digits = 3,
  caption = "Phase 1 — excerpt: Silo B after join (shrinkage index & 1-person %)"
)
```
...
```{r preview-phase1-silo-c, echo=FALSE}
knitr::kable(
  head(silo_c, 6),
  digits = 4,
  caption = "Phase 1 — excerpt: Silo C (non-resident ratio)"
)
```

### Silo D: Income 
**Only 2025 median used, other years are modelled via index**
```{r, echo=TRUE}
inc_raw <- read.csv("Gross monthly income.csv", check.names = FALSE, comment.char = "")
total_row <- which(inc_raw[[1]] == "Total")[1]
if (is.na(total_row) || length(total_row) == 0) total_row <- 11L
...
# ========== Phase 2: Data Cleaning & Normalization (Long Pivot & Standardisation) ==========
...
```{r preview-phase2-housing-shares, echo=FALSE}
knitr::kable(
  head(housing_shares %>% arrange(Year, Series), 10),
  digits = 2,
  caption = "Phase 2 — excerpt: long `housing_shares` (year, series, share %)"
)
```
...
```{r preview-phase3-engineered, echo=FALSE}
knitr::kable(
  head(
    silo_c_eng %>% select(Year, non_resident_ratio, nr_pct_change, high_external_demand),
    6
  ...
  digits = 3,
  caption = "Phase 3 — excerpt: Silo C engineered (non-resident ratio, YoY % change, high-demand flag)"
)
```
...
- **Phase 3:** **Shrinkage index** from Silo B (1- + 2-person) / total; **married_no_children_pct** from Living Alone file counts / total households. **NR impact:** annual % change of non-resident ratio; flag **high_external_demand** when growth > 3%. **Affordability ratio:** median_income_index / condo_price_index. (Silo E—e.g. floor-area index, small-flat supply share—is not included in the current pipeline.)
...
- **Consume:** The report shows the master schema and head/tail in a kable table. Charts use the long **housing shares** from Phase 2 (same denominator per year). Report is rendered to a single HTML (Quarto, `embed-resources: true`); charts are **plotly** (original: one trace per series; improved: 5 panels—4 dwelling small multiples and 1 combined context chart—with direct labels and date filter). Dependencies: `tidyverse`, `plotly`, `htmlwidgets`, `knitr`, Quarto.

## Quality checks {#sec-workflow-quality}
...
# Planned Work Distribution {#sec-work-distribution}
The following table outlines the planned work distribution during the final presentation stage of the project for the Quarto RevealJS Presentation deck and video recording.

| Admin Number | Name | Responsibilities |
|------|--------|--------|
| 2402319 | SIM EN QI | **Data Selection and Sources**: Present the supporting data silos chosen to contextualise and explain why each was relevant. |
| 2402844 | CORVAN CHUA | **Data Engineering Workflow**: Present engineering pipeline on how all five CSVs were extracted and mapped through the phases. Show the master table schema and sample rows. |
| 2502218 | SOH JIA HUI (SU JIAHUI) | **Critiques and Proposed Improvements**: Present the identified weaknesses of the original chart with reference to Tufte's principles. |
| 2501484 | AVRIL LEONG KE EN | **Insights and Analysis**: Interpret the findings from the master table, connecting the engineered variables: shrinkage index, non-resident ratio, affordability ratio. |
| 2501222 | KOK YI HENG | **Conclusion**: Consise Summary. |

: {tbl-colwidths="[30, 35, 90]"}

**Note:** Planned work distribution may subject to changes.

# Summary {#sec-summary}

This report examines a multi-series time-series line chart by the Singapore Department of Statistics tracking resident household proportions across five dwelling types from 1990 to 2024. Key shifts — including declining HDB flat residency and rising condominium living — are attributed to shrinking household sizes, a growing non-resident population, and rising household incomes. The original chart is critiqued for visual clutter and poor accessibility, and one improved **faceted** visualisation (shared year axis, line geoms throughout) with direct labelling and an interactive date filter is proposed. A four-phase data engineering pipeline integrates five data sources into a Master Analytical Table, incorporating engineered features such as a Shrinkage Index, Non-Resident Impact Factor, and Affordability Ratio to support a more comprehensive analysis of Singapore's evolving housing landscape.

# References {#sec-references}
- [1] [CNA — Housing demand, population, URA master plan](https://www.channelnewsasia.com/singapore/housing-demand-population-ura-master-plan-5224401)
- [2] [CNA — Proportion of one-person homes up, shrinking household sizes (HDB survey)](https://www.channelnewsasia.com/singapore/proportion-one-person-homes-up-shrinking-household-sizes-hdb-survey-5491071)
- [3] [Straits Times — Singapore homes are getting smaller](https://www.straitstimes.com/life/youre-not-imagining-it-singapore-homes-are-getting-smaller)
