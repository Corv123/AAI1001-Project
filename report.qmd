...
subtitle: "AAI1001 — Data Engineering and Visualization<br>[Course / Assignment placeholder]"
author:
  name: "[Your Name]"
...
## Understanding the original visualisation {#sec-understanding}
...
This is our dataset's dimensions: **Year** | **Resident Households** | **Total HDB Dwellings** | **HDB 1- and 2-Room Flats** | **HDB 3-Room Flats** | **HDB 4-Room Flats** | **HDB 5-Room and Executive Flats** | **Condominiums and Other Apartments** | **Landed Properties** | **Other Types of Dwelling**. The data for the rest of the columns except Year is represented in count.
...
<style>
/* Keep charts within main content area (respects TOC/sidebar layout) */
.chart-container {
  width: 100%;
  max-width: 100%;
  margin: 1em 0;
...
/* Plotly divs constrained to container, responsive height */
.plotly {
  width: 100% !important;
...
# Calculate percentages relative to Resident Households
...
# Blue shades (light to dark) and line styles (solid, dotted, dashed mix)
blue_shades <- c(
...
p <- p %>%
  ...
  htmlwidgets::onRender("
    function(el, x) {
      // Store reference to the plotly div
      window.plotlyChartDiv = el;
      
      // Make updateChartRange function available globally
      window.updateChartRange = function() {
        ...
        if (startYear < 1980 || endYear > 2024) {
          alert('Year range must be between 1980 and 2024');
          return;
        }
...
Five panels: dwelling shares **HDB 3-room**, **HDB 4-room**, **HDB 5-room**, **Condo** (Silo A), then a **combined context chart** (1+2 person % and Non-Resident % with direct labels — Silos B & C). Condo sits directly above the combined chart. Same x-axis (Year) across all panels. Blue = Public (HDB); warm = Private. Use the date filter below to set the range.
...
```{r improved-chart}
# 4 dwelling panels (Silo A) + 1 combined context panel from master_table (Silos B & C). Landed excluded.
...
p_combined <- plot_ly(mt_ctx) %>%
  add_trace(x = ~Year, y = ~(shrinkage_index * 100), type = "scatter", mode = "lines",
    name = "1- & 2-person households (%)", line = list(width = 2, color = "#4a90a4"),
    hovertemplate = "<b>1+2 person %</b><br>Year: %{x}<br>% of households: %{y:.1f}%<extra></extra>") %>%
  add_trace(x = ~Year, y = ~(non_resident_ratio * 100), type = "scatter", mode = "lines",
    name = "Non-residents (% of population)", line = list(width = 2, color = "#c6531a"),
  ...
  layout(
    ...
    annotations = list(
      list(
        x = last_shrink$Year, y = last_shrink$shrinkage_index * 100, text = "1+2 person",
        showarrow = FALSE, xanchor = "left", xref = "x", yref = "y",
        font = list(size = 10, color = "#4a90a4"), cliponaxis = FALSE
      ),
      list(
        x = last_nr$Year, y = last_nr$non_resident_ratio * 100, text = "Non-Resident",
        showarrow = FALSE, xanchor = "left", xref = "x", yref = "y",
        font = list(size = 10, color = "#c6531a"), cliponaxis = FALSE
      )
    )
  )
...
plist_all <- c(
  plist_dwelling[1:4],           # HDB 3-room, HDB 4-room, HDB 5-room, Condo
  list(p_combined)              # Combined 1+2 person & non-resident (direct labels)
)
p2 <- subplot(plist_all, nrows = 5, margin = 0.02, shareX = TRUE, shareY = FALSE) %>%
  layout(
    title = list(text = "Dwelling shares with Condo above context", x = 0.5, font = list(size = 14)),
  ...
  htmlwidgets::onRender("
    function(el, x) {
      window.updateChartRange2 = function() {
        ...
        var updates = { 'xaxis.autorange': false, 'xaxis.range': [startYear, endYear], 'xaxis.dtick': dtick };
        for (var i = 2; i <= 5; i++) {
          updates['xaxis' + i + '.range'] = [startYear, endYear];
          updates['xaxis' + i + '.dtick'] = dtick;
        }
        Plotly.relayout(el.id, updates);
      };
    }
  ")
p2
...
\
\
Adding on, we can refer to SingStat Table M810651, where we can notice a rising trend of resident households living alone as well as those married without children.

![SingStat Table M810651](images/m810651.png)

### Composition of Non-Residents {#sec-data-non-residents}

This refers to the growth in the composition of non-residents in Singapore, which has largely contributed to that of the growth of Singapore’s entire population. It has been noted by Dr Woo Jun Jie, senior lecturer at the Lee Kuan Yew School of Public Policy, who researches urban governance [x], that this growth has contributed largely in driving demand for private housing, including those for purchase and rental ([CNA: Housing demand](https://www.channelnewsasia.com/singapore/housing-demand-population-ura-master-plan-5224401)). Referring to SingStat Table M810001, we can observe a jump in the non-resident population from 2021 to 2025.
...
Gather related data to support your visualization project. Clearly document the data sources and all steps taken to clean and transform the data. You may choose to find new datasets or use the data corresponding to the original visualization, with justifications. Document the code used for data cleaning and transformation, ensuring reproducibility. The workflow is a **multi-source data engineering pipeline** implemented in **Phases 1–4** below. The report uses **all five project CSVs**; the R code and the short *ETL summary* (Extract / Transform / Load / Quality) at the end of this section describe the same pipeline from two perspectives.

## Phase 1: Multi-Source Extraction & Schema Mapping {#sec-phase1}

Data are pulled from four distinct **data silos**, each mapped to a common key: **[Year]**.

| Silo | Source | Action |
|------|--------|--------|
| **A (Core Housing)** | SingStat M810351 (Resident Households by Dwelling) | Convert absolute counts to **percentage shares** to match the original visualisation. |
| **B (Demographics)** | SingStat M810371 (Household Size) & M810651 (Living Arrangements) | Extract **"1-person"** and **"Married w/o Children"** as new feature columns. |
| **C (Population)** | SingStat M810001 (Residency Status) | Calculate the **Non-Resident Ratio** (Non-Residents / Total Population). |
| **D (Economics)** | MOM Labour Force Report 2025 | Extract **Median Monthly Income** and deflate using CPI to get **Real Income Growth**. |
| **E (Supply-Side)** | data.gov.sg HDB Resale Flat Prices (1990–2026) | Compute **average floor area** by lease year and **small-flat share** of the supply mix. |
...
## Phase 3: Feature Engineering (Creating the "4th Dimension") {#sec-phase3}
...
## Phase 4: Integration (The Master Join) {#sec-phase4}
...
### Pipeline implementation (R code)
...
```{r pipeline-phase1, echo=TRUE, eval=FALSE}
# ========== Phase 1: Multi-Source Extraction & Schema Mapping ==========
# Common key: Year

# Silo A (Core Housing): data.csv
silo_a_wide <- read.csv("data.csv", check.names = FALSE)
year_cols <- setdiff(names(silo_a_wide), "Data Series")
years <- as.numeric(year_cols)

# Silo B (Demographics): Resident Household by household.csv — 1-person, 2-person, total, avg household size
hh_wide <- read.csv("Resident Household by household.csv", check.names = FALSE)
hh_year_cols <- setdiff(names(hh_wide), "Data Series")
hh_long <- hh_wide %>%
  mutate(Series = trimws(`Data Series`)) %>%
...
# Living Alone & Married w/o Children: Living Alone Married without Ch.csv (has year gaps — interpolate later)
alone_wide <- read.csv("Living Alone Married without Ch.csv", check.names = FALSE)
# Keep only top-level Resident Households block (rows 2–7) to avoid duplicating age-group series
alone_wide <- alone_wide %>% slice(2:7)
alone_year_cols <- setdiff(names(alone_wide), "Data Series")
alone_long <- alone_wide %>%
  mutate(Series = trimws(`Data Series`)) %>%
...
living_alone   <- alone_long %>% filter(Series == "Living Alone") %>% select(Year, living_alone = Value)
married_no_ch  <- alone_long %>% filter(Series == "Married Couple-Based Without Children") %>% select(Year, married_no_children = Value)
# Interpolate to full year range so we can join on Year
y_range <- min(silo_b_from_hh$Year, na.rm = TRUE):max(silo_b_from_hh$Year, na.rm = TRUE)
living_alone_full   <- approx(living_alone$Year, living_alone$living_alone,   xout = y_range, rule = 2) %>% as_tibble() %>% rename(Year = x, living_alone = y)
married_no_ch_full  <- approx(married_no_ch$Year, married_no_ch$married_no_children, xout = y_range, rule = 2) %>% as_tibble() %>% rename(Year = x, married_no_children = y)
...
# Silo C (Population): Composition of Non-Residents.csv — Non-Resident Ratio
pop_wide <- read.csv("Composition of Non-Residents.csv", check.names = FALSE)
pop_year_cols <- setdiff(names(pop_wide), "Data Series")
# Ensure year columns are all character so pivot_longer doesn't mix types (some cols read as numeric if they have only numbers)
pop_long <- pop_wide %>%
  mutate(Series = trimws(`Data Series`), across(all_of(pop_year_cols), as.character)) %>%
  pivot_longer(cols = all_of(pop_year_cols), names_to = "Year", values_to = "Value") %>%
  mutate(Year = as.numeric(Year), Value = as.numeric(Value))
...
# Silo D (Economics): Gross monthly income.csv — 2025 median only; no time series in file
inc_raw <- read.csv("Gross monthly income.csv", check.names = FALSE, comment.char = "")
total_row <- which(inc_raw[[1]] == "Total")[1]
if (is.na(total_row) || length(total_row) == 0) total_row <- 11L
inc_vals <- suppressWarnings(as.numeric(gsub(",", "", unlist(inc_raw[total_row, 3:ncol(inc_raw)]))))
...
```{r pipeline-phase2, echo=TRUE, eval=FALSE}
...
```{r pipeline-phase3, echo=TRUE, eval=FALSE}
...
```{r pipeline-phase4, echo=TRUE, eval=FALSE}
...
- **Join:** All silos aligned on **Year**; missing years in one source yield NA in that column (e.g. income only 2025). Silo E joins on `lease_commence_date` as **Year**; lease years outside the dwelling-share range will carry NA for housing columns.

# Team Work Distribution {#sec-work-distribution}

*Placeholder: describe planned or actual work distribution among team members (e.g. Corvan: Show and explain the chosen visualisation and the story to tell; others: data engineering, improved chart, strengths/weaknesses, data sources).*

# Summary {#sec-summary}

This report showed an original multivariate time-series chart of dwelling-type proportions and an improved chart (five panels: four dwelling-type small multiples and one combined context chart for 1+2 person % and Non-Resident %, with direct labels and date filter). Use the date-range controls on each chart to focus on specific years.

# References {#sec-references}

- [CNA — Housing demand, population, URA master plan](https://www.channelnewsasia.com/singapore/housing-demand-population-ura-master-plan-5224401) [x]
- [CNA — Proportion of one-person homes up, shrinking household sizes (HDB survey)](https://www.channelnewsasia.com/singapore/proportion-one-person-homes-up-shrinking-household-sizes-hdb-survey-5491071) [y]
- [Straits Times — Singapore homes are getting smaller](https://www.straitstimes.com/life/youre-not-imagining-it-singapore-homes-are-getting-smaller) [z]
...
- SingStat tables: M810351 (Resident Households by Dwelling), M810371 (Household Size), M810651 (Living Arrangements), M810001 (Residency / Composition of Non-Residents).
- Housing & Development Board. (2021). *Resale Flat Prices* [Dataset collection]. data.gov.sg. <https://data.gov.sg/collections/189/view>
- [Housing Map — HDB flat size analysis](https://www.housingmap.sg/hdb-flat-size-analysis/)
- [PropertyGuru — Are condos and HDB flats getting smaller?](https://www.propertyguru.com.sg/property-guides/are-hdb-flats-condos-getting-smaller-48893)