...
p2 <- subplot(plist_all, nrows = 5, margin = 0.02, shareX = TRUE, shareY = FALSE) %>%
  layout(
    title = list(text = "(Improved Chart) Dwelling shares with Condo above context (same x)", x = 0.5, font = list(size = 14)),
...
Start Year and End Year controls (with an "Update Range" button and Enter-key support) allow the user to focus on a chosen time window (e.g. 1995–2005 or 2010–2024). Narrowing the range makes it easier to read values and compare series without relying on gridlines, and the same filter applies across all five panels (four dwelling + one combined context) so comparison remains consistent.
...
| 2402844 | CORVAN CHUA | • Improved Chart & Data Integration<br>• Created Draft Report<br> |
...
This report critically examines a multi-series time-series line chart published by Singapore Department of Statistics, which tracks the proportion of resident households across five dwelling types from 1990 to 2024. The analysis identifies key structural shifts in Singapore's housing composition, where most notably, a decline in HDB flat residency and a steady rise in condominium living, and attributes these changes to a convergence of demographic, population, and economic factors: shrinking household sizes, a growing share of singles and married-without-children households, an expanding non-resident population, and rising gross monthly incomes. While the original chart effectively conveys long-term trends, it suffers from visual clutter, poor accessibility, and an over-reliance on dash-pattern encoding. In response, an improved visualisation using five panels (four dwelling-type small multiples and one combined context chart for 1+2 person % and Non-Resident %), with direct labels and an interactive date-range filter, is proposed to enhance clarity and analytical depth. Supporting this redesign, a four-phase data engineering pipeline integrates five CSV sources into a Master Analytical Table, incorporating engineered features such as a Shrinkage Index, Non-Resident Impact Factor, and Affordability Ratio to provide a more comprehensive, multi-dimensional view of the forces shaping Singapore's evolving housing landscape.
