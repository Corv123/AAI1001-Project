...
make_opt_a <- function(series_name, title, color) {
  ...
  plot_ly(d, x = ~Year, y = ~Percent, type = "scatter", mode = "lines", line = list(width = 2, color = color),
    ...
    layout(
      ...
      margin = list(t = 36, b = 32, l = 44, r = 20),
      showlegend = FALSE,
      annotations = list(
        list(
  ...
  htmlwidgets::onRender("
    function(el, x) {
      window.updateChartRange2 = function() {
        ...
        var update = { 'xaxis.range': [startYear, endYear], 'xaxis.dtick': dtick };
        for (var i = 2; i <= 5; i++) { update['xaxis' + i + '.range'] = [startYear, endYear]; update['xaxis' + i + '.dtick'] = dtick; }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var endInput = document.getElementById('endYear');
  
  if (startInput) {
    startInput.addEventListener('keypress', function(e) {
      if (e.key === 'Enter') {
        if (window.updateChartRange) {
  ...
  if (startInput2) {
    startInput2.addEventListener('keypress', function(e) {
      if (e.key === 'Enter' && window.updateChartRange2) {
...
## Supply-Side Structural Changes {#sec-data-supply}

A factor not captured by any demand-side analysis is the progressive reduction in HDB flat floor areas over successive building generations. As illustrated in the figure below, the average size of newly built HDB flats has declined substantially since the 1980s and 1990s.

![Supply-side structural changes — HDB flat sizes](images/supply_side_structural_changes.png)
...
The report uses the following CSV files: `data.csv` (dwelling-type counts), `Resident Household by household.csv`, `Living Alone Married without Ch.csv`, `Composition of Non-Residents.csv`, and `Gross monthly income.csv`. Percentages in the charts are computed relative to total resident households for each year. Full citations: Singapore Department of Statistics (SingStat), HDB sample household survey, MOM Labour Force Report.
